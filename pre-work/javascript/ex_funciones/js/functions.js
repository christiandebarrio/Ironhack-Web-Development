...
//	var longestWord = 0;
	for ( i = 0, longestWord = 0; i < array_words.length ; i++ ) {
		if (array_words[i].length > longestWord) {
			longestWord = array_words[i].length;
		}
	}
	return longestWord;
}
...
array_lengths.push( findLongestWord( ["Richie Sampdora", "Joanie Cullum", "Greg Johnson", "Marcia López", "Bobby MacMillan"]) );
...
function beautifyLetters( array )
{
	...
	for (i = 0; i < array.length; i++) {
		...
		} else if ( /[aeiouAEIOU]/.test(array[i]) ) {
...
function beautifyNumbers( array )
{
	...
	for (i = 0; i < array.length; i++) {
		if (isFinite(array[i]) && array[i] > 9) {
			var numberString = array[i].toString().split('');
			while (numberString.length > 1) {
				for (var x = 0, sum = 0; x< numberString.length; x++) {
					sum += Number(numberString[x]);
				}
				array[i] = sum;
				numberString = array[i].toString().split('');
			}
		}
	}
	return array;
